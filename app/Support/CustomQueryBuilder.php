...
//            $filter['match'] = 'and';
    ...
    public function inThePeroid($filter, $query)
    {
        ...
        switch ($filter['query_1']) {
            ...
            case 'next_year':
                ...
                break;
            default:
                $date_range = explode(' to ',$filter['query_2']);
                error_log(json_encode($date_range));
                if (count($date_range)>1){
                    $begin = Carbon::parse($date_range[0])->startOfDay();
                    $end = Carbon::parse($date_range[1])->endOfDay();
                }else{
                    $begin = Carbon::parse($date_range[0])->startOfDay();
                    $end = Carbon::parse($date_range[0])->endOfDay();
                }
                break;
        }
        error_log($begin.'  ----  '.$end);
    ...
    public function customDate($filter, $query){
        $begin = date('Y-m-d',strtotime($filter['query_1']));
        $end = date('Y-m-d',strtotime($filter['query_2']));
        error_log($begin.'  ----  '.$end);
        return $query->whereBetween($filter['column'], [$begin, $end], $filter['match']);
    }
